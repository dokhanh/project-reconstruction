...
//    	System.out.println("Conflicts zone [" + conflicts.size() + "]: " + conflicts);
	...
	/**
	 * Outputs the Voronoi vertex dual to cell c. By definition, this vertex coincides with the circumcenter of c.
	 */
	...
	/**
	 * Outputs the boundary of the Voronoi region dual to vertex v. The Voronoi faces forming the region boundary are triangulated, for visualization purposes.
	 */
	public Collection<Triangle_3> dual (TriangulationDSVertex_3<Point_3> v) {
		...
		for (TriangulationDSCell_3<Point_3> c : ic)
			for (int i=1; i<4; i++) 
				// add edges only if it is not already present
				ie.add(new TriangulationDSVertex_3[]{v, c.vertex((c.index(v)+i)&3)});
		
		LinkedList<Triangle_3> res = new LinkedList<Triangle_3> ();

		// for each edge, compute sorted list of incident cells and then triangulate Voronoi face
		for (TriangulationDSVertex_3<Point_3>[] e : ie) {
			ic = incidentCells(e[0], e[1]);
...
//		new MeshViewer(del);
	...
	private static void test4 () {
		...
		int i=0;
		for (TriangulationDSCell_3<Point_3> c : del.cells()) {
